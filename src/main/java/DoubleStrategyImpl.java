...
public class DoubleStrategyImpl extends BaseStrategyImpl {
    ...
    private boolean shouldLeaveCorner() {
        ...
        if (getTargetingEnemies().size() > 1
                && (nearestCorner == null || self.getDistanceTo(
                        nearestCorner.x, nearestCorner.y) < self.getWidth())) {
            return true;
        }
        return false;
    }
    ...
    private void initMove() {
        ...
        if (x <= self.getHeight() || x >= world.getWidth() - self.getHeight()
                || y <= self.getHeight()
                || y >= world.getHeight() - self.getHeight()) {
    ...
    private void walkMove() {
        ...
        if (!dangerShells.isEmpty()) {
            ...
            Tank enemy = !targetingEnemies.isEmpty()
                ? getNearestTank(targetingEnemies)
                : getNearestTank(enemies);
    ...
    private void oneOnOneMove() {
        ...
        if (!dangerShells.isEmpty()) {
            ...
            drive(bonus);
                    ...
                    || (enemy.getCrewHealth() == self.getCrewHealth() 
                        && enemy.getHullDurability() < self.getHullDurability())) 
        {
            drive(enemy);
